#!perl -w
#
#	Extracts root certificates of Mozilla into separate files
#	Redirecting output into log is good idea
#
#	usage: certdata.perl < certdata.txt > log.log
#
#	certdata.txt is here: http://mxr.mozilla.org/mozilla/source/security/nss/lib/ckfw/builtins/certdata.txt
#
# 
# ***** BEGIN LICENSE BLOCK *****
...
sub dudump2 {
	...
	if (! $val eq '') {
		...
		open(OUT, ">certs/$label.der") || die("could not open $label for write");
		my @bs = split(/\\/, $val);
		foreach my $b (@bs) {
			chomp $b;
		...
		close(OUT);
		# check: openssl x509 -text -inform DER -fingerprint
	}
